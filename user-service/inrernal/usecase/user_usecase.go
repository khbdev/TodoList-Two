	...
	func (s *UserService) Login(ctx context.Context, email, password string) (*model.User, bool, error) {
	if strings.TrimSpace(email) == "" {
		return nil, false, errors.New("email required")
	}
	if strings.TrimSpace(password) == "" {
		return nil, false, errors.New("password required")
	}
	...
	for _, u := range users {
		if strings.ToLower(strings.TrimSpace(u.Email)) == target {
			ok := pkg.CheckPassword(password, u.PasswordHash)
			if !ok {
